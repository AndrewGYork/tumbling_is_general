...
# with different diffusion times). We do that trick here to generate a
# simulated curve for a 50-50 mixture of particules with diffusion time
# 450 and particles with diffusion time 3000.

# path setup & parameter configuration
cwd = Path(__file__).parents[0]
out_dir = cwd.parents[3] / 'tumbling_temp' / 'camera_existingFP_lowerN'
...
results = pd.read_csv('01_pump_probe_n1p0e+05_tqy0p0_9reps_2pulses.csv')
mixed_list = []
...
for i in range(120):
    ...
    ax2.set_yticks(np.linspace(0, 3e2, 4))
    ax2.set_ylim(0, 3e2)
    ax2.ticklabel_format(axis="y", style='sci', scilimits=(0,0))
    ax4.set_yticks(np.linspace(-0.5, 0, 6))
    ax4.set_ylim(-0.5, 0)
    ...
    ax3.set_title(
        r'{:0.0f} x 10$^5$ fluorophores, $\Phi_{{triplet}}$ {:0.2f}'.format(
            n_digit, tqy))
