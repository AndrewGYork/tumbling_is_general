...
# This script simulates how a 'crescent selected' population
# rotationally diffuses over time for different sized objects.

# It models the 'excited state' (i.e. the way the oriented subset was
# generated) as infinitely long lived. The key point to communicate here
# is the fundamentals of rotational diffusion without any photophysics
# layered on top. Nevertheless, a concrete example of such a population
# would be linearly polarized excitation light and perpendiculary
# polarized off-switching or STED light.
...
print("Generic Decay of an Oriented Population from a Crescent Selection")
output_dir = Path(__file__).parents[4]/'tumbling_temp'/'crescent_relaxation'/'individ'
...
for diff in diff2diam.keys():
    ...
    a.phototransition('excited', 'ground', intensity=10,
                      polarization_xyz=(1, 0, 0))
    a.delete_fluorophores_in_state('ground')
    x, y, z = a.get_xyz_for_state('excited')
    avg_x = np.mean(abs(x)); avg_y = np.mean(abs(y))
    x_coord_list = [avg_x]; y_coord_list = [avg_y]
    time_ns_list = [0]
    ...
    for i in range(200):
        ...
        x, y, z = a.get_xyz_for_state('excited')
        avg_x = np.mean(abs(x)); avg_y = np.mean(abs(y))
        x_coord_list.append(avg_x); y_coord_list.append(avg_y)
...
results.to_csv('01_crescent_orientation_relaxation.csv')
    
    
plt.figure()
for name, group in results.groupby('diff_time_ns'):
    plt.plot(group['time_ns'], group['avg_y_coord'],
             color=diff2color[name], label=name)
plt.legend()
plt.savefig('02_crescent_orientation_relaxation.png')
plt.show()
