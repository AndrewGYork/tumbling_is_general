...
# the script 001_generic_decay_crescent and combines them into a series of
# stills with a nice graph and all four different bead sizes on it.

cur = Path(__file__).parents[0]
src = cur.parents[3] / 'tumbling_temp' / 'crescent_relaxation' / 'individ'
out_path = cur.parents[3] / 'tumbling_temp' / 'crescent_relaxation' / 'combined'
Path.mkdir(out_path, exist_ok=True)
mpl.rcParams['mathtext.default'] = 'regular'

# also import the quantification so we can provide a graph
path_csv = cur / '01_crescent_alignment_relaxation.csv'
...
for x in range(201): # number of frames that were previously generated
    ...
    for name, group in results.groupby('diff_time_ns'):
        ax5.plot(group['time_ns'] / 1000,
                 group['avg_y_squared'], label=str(diff2diam[name]) + ' nm',
    ...
    ax5.legend(loc='upper right', title='Diam. (nm)')
