...
## Run this script AFTER the 01_simulate_photobleaching_power script to
...
## It also generates a dots-on-the-sphere animation of orientation for
## two sizes at a given saturation to help build intuition about why the
## power-variation approach works.

# import the simulation output for plotting & some setup
cwd = Path(__file__).parents[0]
adir = cwd.parents[3] / 'tumbling_temp' / 'photobleaching' / 'dots_on_a_sphere'
...
optics = mpimg.imread('photobleaching_optics.png')
...
for i in range(100): # first 10 us only
    ...
    for ax in [ax1, ax3, ax4, ax5, ax6, ax8]:
        ax.set_axis_off()
    # subpanel lettering & indication of which simulation this is
    ...
    for ax, diff in zip([ax3, ax4], ['7p5e+02', '6p0e+03']):
        ...
        ax.imshow(im[155:1155, 250:1250, :])
    c750 = cdfs.loc[cdfs['diffusion_time_ns'] == 750]
    c6000 = cdfs.loc[cdfs['diffusion_time_ns'] == 6000]
    ax3.text(500, 120, 'Monomer', color='#1f77b4', fontsize=10,
             ha='center')
    ax4.text(500, 120, 'Large Aggregate', color='#d62728', fontsize=10,
             ha='center')
    # add the time annotation
    time_us = c750['time_ns'].values[i] / 1000 # assuming 750, 6000 match
    ax3.text(650, -20, r'Time ($\mu$s): {:04.1f}'.format(time_us),
             ha='left', va='bottom', fontsize=10)
    # generate a legend for the dots on a sphere states
    dot1 = plt.Circle((1.3, 0.7), 0.07, color='#808080')
    dot2 = plt.Circle((1.3, 0.25), 0.07, color='#23e000')
    ax5.add_artist(dot1); ax5.add_artist(dot2)
    ax5.set_aspect('equal')
    ax5.set_ylim(0,1); ax5.set_xlim(0,5)
    ax5.text(1.6, 0.7, 'ground state', va='center')
    ax5.text(1.6, 0.25, 'excited state', va='center')
    ...
    ax2.plot(1.25, y2, marker='s', markeredgecolor='#d62728', markersize=8,
             markerfacecolor='none', markeredgewidth=1.5, alpha=0.5)
    xyA1 = [50, 550] # coordinates on the animation
    ...
    xyA2 = [50, 675] # coordinates on the animation
    ...
    ax7.bar(range(2), [c750['y_cdf'].values[i], c750['x_cdf'].values[i]],
    ...
    ax9.bar(range(2), [c6000['y_cdf'].values[-1], c6000['x_cdf'].values[-1]],
            fill=None, edgecolor='#808080')
    ax9.bar(range(2), [c6000['y_cdf'].values[i], c6000['x_cdf'].values[i]],
