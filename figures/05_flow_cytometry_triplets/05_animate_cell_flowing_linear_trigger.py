...
out_dir = cwd.parents[2] / 'tumbling_temp' / 'flow_sequence_linear'
out_dir.mkdir(exist_ok=True, parents=True)
im_list = []
for i in range(5):
    im = mpimg.imread(cwd / 'flow_stills_linear' / ('%03d.png' % (i+1)))
    im_list.append(im)
im = np.array(im_list)
print(im.shape)
...
results = pd.read_csv('flow_cytometry_linear_probe_12reps.csv')
...
for i, x in enumerate(range(0, 900, 5)):
    ...
    # generate a rectangle that highlights where we are measuring
    if 25 < x <= 95: # delay 1
        box = plt.Rectangle((185, -0.35), 30, 0.2, fill=False,
                            edgecolor='#ec008c')
        ax3.add_artist(box)
    elif 248 < x <= 395: # delay 2
        box = plt.Rectangle((750, -0.385), 100, 0.1, fill=False,
                            edgecolor='#ec008c')
        ax3.add_artist(box)
    elif 555 < x <= 805: # delay 3
        box = plt.Rectangle((3000, -0.39), 400, 0.04, fill=False,
                            edgecolor='#ec008c')
        ax3.add_artist(box)        
