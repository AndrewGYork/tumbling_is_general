...
# FIGURE 1: CAP SELECTION ONLY, MEAN OF 12 REPLICATES
...
for name, group in cap.groupby('diffusion_time_ns'):
    ...
    axs1.plot(delays, pol_means, '.-', markersize=5,
              label=str(diff2diam[name])+' nm',
              color=diff2color[name])
    axs1.fill_between(delays,
                      pol_means - pol_stds,
                      pol_means + pol_stds,
                      color=diff2color[name], alpha=0.2)
plt.legend(title='Diameter (nm)', ncol=2)
...
plt.savefig('04_simulation_cap_selection_12reps.pdf', bbox_inches='tight',
            transparent=True)

# FIGURE 2: EFFECT OF CRESCENT SELECTION ON THE DYNAMIC RANGE
...
for ax in fig2.axes:
    ...
    ax.set_ylim(-0.5, 0.5)
plt.xticks(range(0, 1250, 250))
plt.savefig('05_simulation_varyingCrescent_pump2_probe0p25.pdf', bbox_inches='tight',
            transparent=True)

# FIGURE 3: EFFECT OF CRESCENT SELECTION ON TOTAL COUNTS
...
plt.savefig('06_signal_loss_from_crescent.pdf', bbox_inches='tight',
            transparent=True)

# FIGURE 4: PLOT THE CRESCENT CONDITION TO MATCH MSCARLET EXPERIMENT DATA
# Based on the fraction of counts lost from crescent selection, I
# estimate that the 2% 775 crescent corresponds to a saturation of
# something like 1.25 for mScarlet. I'll isolate that data and plot it.
cresS = cres.loc[cres['crescent_saturation'] == 1.25]
fig4 = plt.figure(figsize=(4,4), dpi=300)
h = [Size.Fixed(1.0), Size.Fixed(2)]
v = [Size.Fixed(0.7), Size.Fixed(2)]
divider = Divider(fig4, (0, 0, 1, 1), h, v, aspect=False)
axs4 = fig4.add_axes(divider.get_position(),
                     axes_locator=divider.new_locator(nx=1, ny=1))
for name, group in cresS.groupby('diffusion_time_ns'):
    pol_means = group.groupby('delay_us')['polarization'].mean()
    pol_stds = group.groupby('delay_us')['polarization'].std()
    delays = group.groupby('delay_us').groups.keys()
    axs4.plot(delays, pol_means, '.-', markersize=5,
              label=str(diff2diam[name])+' nm',
              color=diff2color[name])
    axs4.fill_between(delays,
                      pol_means - pol_stds,
                      pol_means + pol_stds,
                      color=diff2color[name], alpha=0.2)
plt.title('Simulation')
for ax in fig4.axes:
    ...
    ax.set_xlim(-25, 1025)
plt.xticks(range(0, 1250, 250))
plt.savefig('07_simulation_to_match_mScarlet.pdf', bbox_inches='tight',
            transparent=True)

# FIGURE 5: PLOT THE CRESCENT CONDITION TO MATCH VENUS EXPERIMENT DATA
# Based on the fraction of counts lost from crescent selection, I
# estimate that the 0.5% 775 crescent corresponds to a saturation of
# something like 0.25 for mVenus. I'll isolate that data and plot it.
...
for name, group in cresV.groupby('diffusion_time_ns'):
    ...
    pol_stds = group.groupby('delay_us')['polarization'].std()
    delays = group.groupby('delay_us').groups.keys()
    axs5.plot(delays, pol_means, '.-', markersize=5,
              label=str(diff2diam[name])+' nm',
              color=diff2color[name])
    axs5.fill_between(delays,
                      pol_means - pol_stds,
                      pol_means + pol_stds,
                      color=diff2color[name], alpha=0.2)
plt.title('Simulation')
for ax in fig5.axes:
    ...
    ax.set_xlim(-25, 1025)
plt.xticks(range(0, 1250, 250))
plt.savefig('08_simulation_to_match_mVenus.pdf', bbox_inches='tight',
            transparent=True)
